...
def reportdiffs(unique1, unique2, dir1, dir2):
    if not (unique1 or unique2):
        print('Directory lists are indentical')
    else:
        if unique1:
...
def comparefilesexactly(file1, file2):
    b1 = open(file1, 'rb').read()
    b2 = open(file2, 'rb').read()
    for (i, (b11, b22)) in enumerate(zip(b1, b2)):
        if b11 != b22:
            print(i, repr(b11), repr(b22))
            break
